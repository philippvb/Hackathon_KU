...
class DQNAgent(object):
    ...
    def __init__(self, observation_space, action_space, action_space_size, channels, **userconfig):

        if not isinstance(observation_space, spaces.box.Box):
            print("unsupported ibservation space")
...
#                                    ' (Reqire Discrete.)'.format(action_space, self))
        ...
        self.Q = QFunction(observation_dim=self._observation_space.shape[0],
                  action_dim=self._action_n, learning_rate=self._config["learning_rate"])
        # Q Network
        self.Q_target = QFunction(observation_dim=self._observation_space.shape[0],
                                   action_dim=self._action_n, learning_rate=0)
